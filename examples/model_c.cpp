...
int main(int argc, char *argv[]) {
    ...
    save_model_loc = "/app/model/model_c/";
    model_name = "model_c";
    ...
    for(int epoch = 0; epoch < total_epoches; epoch++){
        ...
        {
            // Create a tmp_cipher for padding
            int input_padding = 31;
            vector<vector<vector<Ctx>>> tmp_input_cipher(input_padding, vector<vector<Ctx>>(input_padding, vector<Ctx>(l1_ic)));
            pg_rt.padding(tmp_input_cipher, D0_cipher, 1, 1);
            // Create a tmp_cipher for catching output values
            vector<vector<vector<Ctx>>> tmp_output_cipher(l1_conv, vector<vector<Ctx>>(l1_conv, vector<Ctx>(l1_kc)));

            pg_rt.conv(pp, tmp_input_cipher, W1, B1, tmp_output_cipher, 2, 1, epoch);

            // Flatten
            pg_rt.flatten(tmp_output_cipher, U1_cipher);

            release_vector_3D(tmp_input_cipher);
            tmp_input_cipher.clear();
        ...
        // Update model weights of Layer 1
        {
            ...
            vector<vector<vector<Ctx>>> tmp_input_cipher(input_padding, vector<vector<Ctx>>(input_padding, vector<Ctx>(l1_ic)));
            pg_rt.padding(tmp_input_cipher, D0_cipher, 1, 1);
            // Create a tmp_cipher for data dilation
            int l1_dilate = l1_conv + (l1_conv - 1) * (2 - 1);
            vector<vector<vector<Ctx>>> tmp_cipher(l1_dilate, vector<vector<Ctx>>(l1_dilate, vector<Ctx>(l1_kc)));
            pg_rt.delta_1D_to_3D_dilate(pp, DE1_cipher, tmp_cipher, l1_conv, 2, 4);

            pg_rt.update_model_conv_with_3D_delta(pp, tmp_input_cipher, tmp_cipher, W1, B1, 5, 1, epoch);

            release_vector_3D(tmp_input_cipher);
            tmp_input_cipher.clear();
            release_vector_3D(tmp_cipher);
            tmp_cipher.clear();
        }
