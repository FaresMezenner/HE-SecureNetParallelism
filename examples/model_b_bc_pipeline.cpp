/*
 * model_b_bc_pipeline.cpp
 * =======================
 * Pipeline-parallel HE training with FLEXIBLE BUFFER (consumption counter).
 * Supports arbitrary TOTAL_STEPS without static_assert guard.
 *
 ...
 *   Thread A (own step loop): L1 fwd → wait buf free → copy D1 → signal → AL + BP
 *   Thread B (own step loop): wait D1 ready → copy to local → signal consumed → L2-L4 + BP
 *
 * BUFFER PROTOCOL (producer-consumer with double buffer):
 *   - Thread A (producer) WAITS at copy step until the target buf[step%2] has been
 *     consumed by Thread B (d1_consumed_step >= step - 2). For step 0 and 1, no wait.
 *   - Thread A writes D1 into buf[step%2], then signals d1_ready_step = step.
 *   - Thread B (consumer) WAITS until d1_ready_step >= step.
 *   - Thread B deep-copies buf into its own local D1, then signals d1_consumed_step = step.
 *   - This frees buf[step%2] for Thread A to reuse 2 steps later.
 *
 ...
 * SEAL MemoryManager::GetMMProf() patched to thread_local.
 * Timing globals in pegasus_runtime.cc are thread_local.
 *
...
static constexpr int TOTAL_STEPS = 2;  // training steps — NOW ARBITRARY
...
struct D1PipelineBuffer
{
    ...
    void wait_buf_free(int step)
    {
        ...
        // buf[step%2] was last written at (step - 2).
        // It's safe once B consumed step - 2.
...
int main(int /*argc*/, char * /*argv*/[])
{
    ...
    TSLogger tlog;
    {
        ...
        std::string log_path = log_dir + "/model_b_pipe_" + std::string(buf) + ".log";
        tlog.logfile.open(log_path, std::ios::out);
        tlog.log("MAIN", "Log file: " + log_path);
    }

    CSVRecorder csv;
    tlog.log("MAIN", "=== Pipeline-parallel experiment (flexible buffer) ===");
    tlog.log("MAIN", "REAL parallelism via std::thread with consumption counter");
    ...
    // ── Model save location ─────────────────────────────────────────
    save_model_loc = "./model/model_b_pipe/";
    model_name = "pipe";
    ...
    // Module A intermediates (used by Thread A only)
...
#pragma omp parallel for
    ...
    //  Works for ANY TOTAL_STEPS value.
    ...
    //            Runs its OWN step loop.
    //            Only halts at copy step if buf[step%2] not yet consumed.
    // ══════════════════════════════════════════════════════════════════
    std::thread thread_a([&]()
                         {
        omp_set_num_threads(NUM_THREADS);
        ...
        for (int step = 0; step < TOTAL_STEPS; ++step)
        {
            ...
            // ─── L1 Forward (Thread A owns D0/W1/D1 exclusively) ─────
            ...
            //   For step 0,1: trivially free (nothing written yet)
            //   For step 2+: wait until B consumed step-2
    ...
    std::thread thread_b([&]()
        ...
        for (int step = 0; step < TOTAL_STEPS; ++step)
        {
            ...
            //   After this copy, Thread A is free to overwrite buf[step%2]
            //   for step+2. Thread B uses D1_local_B for all subsequent ops.
            ...
            timed_op(csv, tlog, "B", "B", "dense", 2, step, "FF", pg_b, [&]
                     { pg_rt.dense(pp, D1_local_B, W2, B2, U2_cipher, 2, epoch); });
            ...
            timed_op(csv, tlog, "B", "B", "update_W2", 2, step, "UPDATE", pg_b, [&]
                     { pg_rt.update_model_dense(pp, D1_local_B, DE2_cipher,
                                                W2, B2, 5, 2, epoch); });
    ...
    // Write CSV
    std::string csv_path = "./logs/pipeline_ops.csv";
    csv.write(csv_path);
