...
void LWEGateBooter::SetPostMultiplier(const double multiplier) {
  if (multiplier != post_multiplier) {
    ...
    InitLUT(LUTs["HalfAbs"], degree, [](double e) { return std::abs(e) > 0.01 ? 0.5 * std::abs(e) : 0; });
    InitLUT(LUTs["Half"], degree, [](double e) { return 0.5 * e; });
    ...
    InitLUT(LUTs["IsNegative"], degree,
            [](double e) { return std::signbit(e) ? 1. : 0.; });
    InitLUT(LUTs["IsNotNegative"], degree,
            [](double e) { return std::signbit(e) ? 0. : 1.; });
    InitLUT(LUTs["AbsLog"], degree,
            [](double e) { return std::log(std::abs(e)); });
    InitLUT(LUTs["Log"], degree,
    ...
    InitLUT(LUTs["Exponent"], degree, [](double e) {
      return e <= -16.0 ? 0.0 : (e >= 7.0 ? 1096. : std::exp(e));
      // return e <= -8.0 ? 0.0 : (e >= 5.0 ? 148. : std::exp(e));
    });
  }
}
...
LWEGATEBOOT_FUNC(HalfAbs, "HalfAbs")
LWEGATEBOOT_FUNC(Half, "Half")
...
LWEGATEBOOT_FUNC(IsNotNegative, "IsNotNegative")
LWEGATEBOOT_FUNC(AbsLog, "AbsLog")
LWEGATEBOOT_FUNC(Log, "Log")
