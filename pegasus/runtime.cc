...
struct RunTime::Impl {
  ...
  Status DropModuliP(Ptx* pt, Ctx* ct) const {
    assert(nToDrop >= 0);
    assert(ct && GetNModuli(*ct) > nToDrop);
    ...
    */
    ...
    try {
        ...
        /*
        for(int i = 0; i < nToDrop; i++) {
            evaluator_->mod_switch_to_next_inplace(*ct);
        }
        */
    } catch (std::logic_error e) {
      return Status::InternalError("KeepLastModuli logic_error: " +
...
Status RunTime::DropModuliP(Ptx* pt, Ctx* ct) const {
  return impl_->DropModuliP(pt, ct);
}
